...
# 1. Dependencies: Node, Chromium, CEC tools, and a Wayland kiosk compositor.
if ! command -v node >/dev/null 2>&1; then
  echo "==> Installing Node.js 20.x"
  curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
  apt-get install -y nodejs
fi
apt-get install -y chromium-browser cec-utils cage || \
  apt-get install -y chromium cec-utils cage
